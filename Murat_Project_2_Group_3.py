...
1 Calculates and adds the average grade for each student to the dictionary.
2 Finds and prints the student with the highest average grade.
3 Separates first names and last names, stores them in a tuple, and adds them to a list.
4 .Sorts the list of names alphabetically and prints the result.
5 Stores students with an average grade below 70 in a set."""

# Using a dictionary to store information and grades of 10 students. Each student has a name, surname, and grades (Midterm, Final, and Oral).
students = {
    'Ahmet Yilmaz': [85, 90, 78],
    'Mehmet Demir': [92, 88, 76],
    'Ayse Kaya': [78, 89, 95],
    'Murat Yildirim': [82, 85, 87],
    'Furkan Bey': [84, 86, 88],
    'Beyza Hanim': [85, 87, 89],
    'Mavis Hanim': [85, 88, 90],
    'Veli Gocer': [65, 57, 55],
    'Emir Tekcan': [87, 90, 95],
    'Kemal Isik': [68, 25, 80]
}

# 1- Calculate and add the average grade for each student to the dictionary
for student, grades in students.items():
    average_grade = sum(grades) / len(grades)
    students[student].append(average_grade)
    

# 2- Find and print the student with the highest average grade
max_average_student = max(students, key=lambda x: students[x][-1])
print(f"The student with the highest average grade: {max_average_student}")

# 3- Separate first names and last names, store them in a tuple, and add to a list
names_list = [(name.split()[0], name.split()[1]) for name in students.keys()]

# 4- Sort the list alphabetically and print
sorted_names = sorted(names_list, key=lambda x: (x, x[0]))
...
print("Students with an average grade below 70:", below_70_set)
